# Mulle Test Documentation Process Guide

## Overview

This document outlines the systematic process for creating comprehensive command documentation for mulle-test. Based on practical experience with documentation creation, it provides clear guidelines for what to write, what to avoid, and the step-by-step process to follow.
...
- **MANDATORY: Analyze the main script's main function directly** - Open the main script file (typically `{mulle-test}/{mulle-test}`) and examine the `{mulle-test}::main()` function's case statement to extract ALL available commands
...
{mulle-test} <command> <arguments>
{mulle-test} <command> --help
{mulle-test} <command> --verbose

# Good: Workflow examples
{mulle-test} <command1> <args>
{mulle-test} <command2> <args>
{mulle-test} <command3> <args>
```
...
- Core Testing Operations (`run`, `craft`, `init`)
- Test Execution (`crun`, `rerun`, `retest`)
- Test Management (`clean`, `recraft`)

### Medium Priority Commands
- System Information (`arch`, `libexec-dir`, `uname`, `version`)
- Environment Management (`env`)
- Advanced Testing (`coverage`, `craftorder`, `linkorder`)

### Low Priority Commands
- Utility Commands (`fetch`, `log`)
- Experimental or advanced features
- Hidden/utility commands</search>
...
### Pre-Documentation
- [ ] **MANDATORY: Read the main script file** (`{mulle-test}/{mulle-test}`) and examine the `{mulle-test}::main()` function's case statement
...
- [ ] `mulle-test run <args>` : Run tests, crafts beforehand if needed
- [ ] `mulle-test craft <args>` : Force rebuild of project, then run tests
- [ ] `mulle-test init <args>` : Setup a test project for the current project

#### [Test Execution]
- [ ] `mulle-test crun <args>` : Craft library if needed, then run a single test
- [ ] `mulle-test rerun <args>` : Run failing tests again
- [ ] `mulle-test retest <args>` : Clean gravetidy, then craft and run

#### [Test Management]
- [ ] `mulle-test clean <args>` : Remove dependency, kitchens and exe files
- [ ] `mulle-test recraft <args>` : Clean all, then craft
- [ ] `mulle-test test-dir <args>` : Locate test folder

#### [System & Info]
- [ ] `mulle-test arch <args>` : Name of current architecture
- [ ] `mulle-test libexec-dir <args>` : Print path to mulle-test libexec
- [ ] `mulle-test uname <args>` : mulle-test's simplified uname(1)
- [ ] `mulle-test version <args>` : Show version information
- [ ] `mulle-test env <args>` : Show environment variables

#### [Additional Commands]
- [ ] `mulle-test coverage <args>` : Generate coverage information
- [ ] `mulle-test craftorder <args>` : Show craft order
- [ ] `mulle-test fetch <args>` : Fetch dependencies
- [ ] `mulle-test linkorder <args>` : Show link order
- [ ] `mulle-test log <args>` : Show logs</search>
</search_and_replace>
...
### Notes
- This is a new documentation session for mulle-test
...
- Documentation structure follows the same pattern as other tools for consistency</search>
